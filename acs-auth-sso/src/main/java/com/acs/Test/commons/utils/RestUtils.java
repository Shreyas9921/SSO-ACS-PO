...
import com.acs.Test.commons.logger.LogManager;
import com.acs.Test.commons.logger.Logger;
import com.acs.Test.commons.logger.MessageType;
...
public class RestUtils {
	...
	public static <T> ResponseEntity<RestResponse<T>> successResponse(T data, String message, HttpStatus statusCode) {
		...
		Logger.getLogger(RestUtils.class).setValue(Logger.getLogger(RestUtils.class).getValue(Constant.REQUEST_ID),
	...
	public static <T> ResponseEntity<RestResponse<String>> errorResponseStr(String errorMessage, HttpStatus statusCode) {
		LogManager.info(RestUtils.class, statusCode.name() + " - " + statusCode.value(),
				MessageType.ApiRequestResponse);
		LogManager.error(RestUtils.class, errorMessage, MessageType.Error);
		Logger.getLogger(RestUtils.class).setValue(Logger.getLogger(RestUtils.class).getValue(Constant.REQUEST_ID),
				String.valueOf(statusCode.value()));
		return new ResponseEntity<>(new RestResponse<String>(errorMessage, null, Boolean.FALSE, statusCode.value()),
				statusCode);
	}

	public static <T> ResponseEntity<RestResponse<Object>> errorResponseObj(String errorMessage, HttpStatus statusCode) {
		LogManager.info(RestUtils.class, statusCode.name() + " - " + statusCode.value(),
				MessageType.ApiRequestResponse);
		LogManager.error(RestUtils.class, errorMessage, MessageType.Error);
		Logger.getLogger(RestUtils.class).setValue(Logger.getLogger(RestUtils.class).getValue(Constant.REQUEST_ID),
				String.valueOf(statusCode.value()));
		return new ResponseEntity<>(new RestResponse<Object>(errorMessage, null, Boolean.FALSE, statusCode.value()),
				statusCode);
	}
